...
export const TableEditDialog = ({ isOpen, onClose, table, onConfirm }: TableEditDialogProps) => {
  ...
  return (
    <Dialog open={isOpen} onOpenChange={(open) => !open && onClose()}>
      <DialogContent className="sm:max-w-[800px] p-0 overflow-hidden border-border/50 shadow-2xl gap-0">
        ...
        <div className="px-6 pt-4 bg-background flex flex-col md:flex-row gap-6">
            ...
            <Tabs defaultValue="dimensions" className="w-full pb-2 flex-1">
              ...
              <TabsContent value="dimensions" className="space-y-4 mt-0 outline-none">
                <div className="flex flex-col gap-4 p-4 rounded-xl border border-border/50 bg-muted/20">
                  <div className="flex items-center bg-muted/50 p-1 rounded-lg border border-border/50 w-full">
                    <button
                  ...
                  <div className="flex items-center justify-between bg-background rounded-lg border border-border/50 p-1 shadow-sm transition-colors focus-within:border-primary/50 focus-within:ring-1 focus-within:ring-primary/50">
                    <Button 
                      variant="ghost" 
                      size="icon" 
                    ...
                    <Input 
                      ...
                      min={1}
                      max={20}
                      className="h-8 w-16 border-0 bg-transparent text-center text-sm sm:text-base font-medium focus-visible:ring-0 focus-visible:ring-offset-0 px-0" 
          ...
          <div className="w-full md:w-1/2 pb-6 order-1 md:order-2">
            ...
            <div className="space-y-3 h-full flex flex-col">
              ...
              <div className="border border-border/50 rounded-xl p-4 bg-muted/20 overflow-auto flex-1 min-h-[250px] relative group">
                      ...
                      {Array.from({ length: Math.max(0, previewRows - 1) }).map((_, r) => (
                        <tr key={r} className="transition-colors hover:bg-muted/30">
                          {Array.from({ length: previewCols }).map((_, c) => (
                            <td key={c} className={`p-3 text-xs text-muted-foreground/70 whitespace-nowrap ${isRounded ? `border-b border-r last:border-r-0 ${r === Math.max(0, previewRows - 1) - 1 ? 'border-b-0' : ''} border-border/50` : 'border border-border/50'}`}>
